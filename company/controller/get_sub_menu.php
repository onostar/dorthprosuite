<?php
    ...
    $rows = $get_item->fetch_details_cond('sub_menus', 'sub_menu_id', $id);
     if(gettype($rows) == 'array'){
        ...
        //get package name
        $pk = $get_menu->fetch_details_group('packages', 'package', 'package_id', $row->package);
        $package_name = $pk->package;
    ?>
            ...
            <div class="inputs">
                ...
                <div class="data" style="width:30%">
                    ...
                    <select name="packages" id="packages">
                        <option value="<?php echo $row->package?>"><?php echo $package_name?></option>
                        <?php
                            //get packages
                            $pkgs = $get_dep->fetch_details_order('packages', 'package');
                            foreach($pkgs as $pkg){
                        ?>
