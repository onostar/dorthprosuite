<?php
    ...
    class selects extends Dbh{
        ...
        // fetch chats
        public function fetch_chats($sender, $recipient){
            $get_user = $this->connectdb()->prepare("SELECT * FROM chat WHERE sender = $sender AND recipient = $recipient OR sender = $recipient AND recipient = $sender ORDER BY chat_time");
            /* $get_user->bindValue("sender", $sender);
            $get_user->bindValue("recipient", $recipient); */
        ...
        public function fetch_details_order($table, $order){
         ...
         //fetch details from any table orderly with condition
        public function fetch_details_orderCond($table, $con, $val, $order){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $con = :$con ORDER BY $order");
        ...
        //fetch details with like or close to with a condition
        public function fetch_details_like1Cond($table, $column, $value, $con, $con_value){
        ...
        // fetch details like 2 conditions and 1 major condition
        public function fetch_details_like2Equal1Cond($table, $column1, $column2, $value, $con, $val2){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $con = :$con AND $column1 LIKE '%$value%' OR $column2 = :$column2");
            $get_user->bindValue("$column2", $value);
            $get_user->bindValue("$con", $val2);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
                return $rows;
        ...
        //fetch details count with 1 positive and negative condition
        public function fetch_count_2cond1neg($table, $column, $condition, $col2, $val2){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $column = :$column AND $col2 != :$col2");
            $get_user->bindValue("$column", $condition);
            $get_user->bindValue("$col2", $val2);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                return $get_user->rowCount();
            }else{
        ...
        public function fetch_count_3cond($table, $column1, $condition1, $column2, $condition2, $con3, $val3){
        ...
        //fetch details count with 2 condition and 1 negative
        public function fetch_count_3cond1neg($table, $column1, $condition1, $column2, $condition2, $con3, $val3){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $column1 = :$column1 AND $column2 = :$column2 AND $con3 != :$con3");
            $get_user->bindValue("$column1", $condition1);
            $get_user->bindValue("$column2", $condition2);
            $get_user->bindValue("$con3", $val3);
        ...
        //fetch details count with 2 condition and current date and grouped by
        public function fetch_count_3condDateGro($table, $column1, $condition1, $column2, $condition2, $con3, $val3, $date, $group){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $column1 = :$column1 AND $column2 = :$column2 AND $con3 = :$con3 AND date($date) = CURDATE() GROUP BY $group");
            $get_user->bindValue("$column1", $condition1);
            $get_user->bindValue("$column2", $condition2);
            $get_user->bindValue("$con3", $val3);
            $get_user->execute();
        ...
        public function fetch_details_3eithercond($table, $condition1, $condition2, $value1, $value2, $value3){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $condition1 = :$condition1 AND ($condition2 = :val2 OR $condition2 = :val3)");
            $get_user->bindValue("$condition1", $value1);
            $get_user->bindValue("val2", $value2);
            $get_user->bindValue("val3", $value3);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
                return $rows;
        ...
        //fetch with four  condition 
        public function fetch_details_4eithercond($table, $condition1, $condition2, $value1, $value2, $value3, $value4){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $condition1 = :$condition1 AND ($condition2 = :val2 OR $condition2 = :val3 OR $condition2 = :val4)");
            $get_user->bindValue("$condition1", $value1);
            $get_user->bindValue("val2", $value2);
            $get_user->bindValue("val3", $value3);
            $get_user->bindValue("val4", $value4);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
                return $rows;
        ...
        //fetch with four condition
        public function fetch_details_4Cond($table, $condition1, $condition2, $cond3, $value1, $value2, $value3, $val4){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $condition1 = :$condition1 AND $condition2 = :$condition2 AND ($cond3 = :val3 OR $cond3 = :val4)");
            $get_user->bindValue("$condition1", $value1);
            $get_user->bindValue("$condition2", $value2);
            $get_user->bindValue("val3", $value3);
            $get_user->bindValue("val4", $val4);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
                return $rows;
        ...
        //fetch with five condition
        public function fetch_details_5Cond($table, $condition1, $condition2, $cond3, $value1, $value2, $value3, $val4, $val5){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $condition1 = :$condition1 AND $condition2 = :$condition2 AND ($cond3 = :val3 OR $cond3 = :val4 OR $cond3 = :val5)");
            $get_user->bindValue("$condition1", $value1);
            $get_user->bindValue("$condition2", $value2);
            $get_user->bindValue("val3", $value3);
            $get_user->bindValue("val4", $val4);
            $get_user->bindValue("val5", $val5);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
                return $rows;
        ...
        //fetch with one condition group by
        public function fetch_AllStock($company){
            $get_user = $this->connectdb()->prepare("SELECT SUM(DISTINCT quantity) AS total, cost_price, item FROM inventory WHERE company = :company GROUP BY item");
            $get_user->bindValue('company', $company);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
                return $rows;
        ...
        public function fetch_details_curdateCon($table, $column, $condition, $value){
         ...
         //fetch all item grouped by a column with condition
         public function fetch_single_groupedCon($table, $group, $con, $val){
            $get_details = $this->connectdb()->prepare("SELECT * FROM $table WHERE $con = :$con GROUP BY $group");
            $get_details->bindValue("$con", $val);
            $get_details->execute();
            if($get_details->rowCount() > 0){
                $row = $get_details->fetchAll();
                return $row;
         ...
         //fetch all item grouped by a column with condition
         public function fetch_single_grouped2Con($table, $group, $con, $val, $val2){
            $get_details = $this->connectdb()->prepare("SELECT * FROM $table WHERE $con = :val1 OR $con = :val2 GROUP BY $group");
            $get_details->bindValue("val1", $val);
            $get_details->bindValue("val2", $val2);
            $get_details->execute();
            if($get_details->rowCount() > 0){
                $row = $get_details->fetchAll();
                return $row;
         ...
         //fetch all item grouped by a column with 2condition
         public function fetch_single_grouped3Con($table, $group, $con, $val, $val2, $val3){
            $get_details = $this->connectdb()->prepare("SELECT * FROM $table WHERE $con = :val1 OR $con = :val2 OR $con = :val3 GROUP BY $group");
            $get_details->bindValue("val1", $val);
            $get_details->bindValue("val2", $val2);
            $get_details->bindValue("val3", $val3);
            $get_details->execute();
            if($get_details->rowCount() > 0){
                $row = $get_details->fetchAll();
                return $row;
        ...
        function fetch_reorder_level($store){
            $get_item = $this->connectdb()->prepare("SELECT SUM(quantity) as total_quantity, reorder_level, item, cost_price FROM inventory WHERE store = :store GROUP BY item, reorder_level HAVING SUM(quantity) <= reorder_level");
        ...
        //fetch details with date and 2 conditions
        public function fetch_details_date3Cond($table, $column, $condition1, $value1, $condition2, $value2, $con3, $val3){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $condition1 = :$condition1 AND $condition2 = :$condition2 AND $con3 =:$con3 AND $column = CURDATE()");
            $get_user->bindValue("$condition1", $value1);
            $get_user->bindValue("$condition2", $value2);
            $get_user->bindValue("$con3", $val3);
            // $get_user->bindValue("$condition2", $value2);
