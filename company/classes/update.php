...
date_default_timezone_set("Africa/Lagos");
    class Update_table extends Dbh{
        ...
        //update password
        public function updateUsername($username, $current_password, $user_id, $full_name){
            ...
            if($get_pwd->rowCount() > 0){
                ...
                $confirm_password = password_verify($current_password, $user_password->user_password);
                if($confirm_password == false){
                    echo "<p class='exist'>Password is Incorrect!</p>";
                }else{
                    
                    $update_password = $this->connectdb()->prepare("UPDATE users SET username = :username, full_name = :full_name WHERE user_id = :user_id");
                    $update_password->bindValue("user_id", $user_id);
                    $update_password->bindValue("username", $username);
                    $update_password->bindValue("full_name", $full_name);
                    $update_password->execute();

                    if($update_password){
                        echo "<p>Profile updated Successfully<br>Kindly Reload the page to sign in again</p>";
                        // session_start();
                        session_unset();
                        session_destroy();
